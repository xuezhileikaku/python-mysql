...
def insert_table(arr, post_json):
    ...
    # sql = "INSERT INTO list (id, name,search_json,) VALUES (%s, %s,)"
    # val = [arr['cid'], arr['name']]    
    sql = "INSERT INTO list(id,name,search_json,create_at,status,content) \
       VALUES ('%s', '%s', '%s','%d','%d','%s')" % \
       (arr['cid'], arr['name'],json.dumps(json.loads(post_json)),time.time(),1,json.dumps(arr))
    try:
...
def update_table(arr, post_json):
    ...
    mydb = mysql.connector.connect(
        host="localhost", user="root", passwd="", database="db")
    mycursor = mydb.cursor()
    sql = "UPDATE list SET search_json = '"+json.dumps(json.loads(post_json))+"',create_at= "+str(time.time())+",status= 1,content= '"+json.dumps(arr)+"' WHERE name = '" + \
        arr['name']+"'"
...
def sele_table(arr):
    mydb = mysql.connector.connect(
        host="localhost", user="root", passwd="", database="db")
    mycursor = mydb.cursor()
    sql = "SELECT * FROM list WHERE name='"+arr['name']+"'"
